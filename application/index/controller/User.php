...
//        dump($loglist);die;
    ...
    public function blessings_transfer()
    {
        ...
        if ($this->request->isPost()) {
            ...
            //减少当前用户
            moneyLog($this->user_id,$tranUser['id'],'pay_points',-$number,8,'萝卜转增');
            //增加转让用户
            moneyLog($tranUser['id'],$this->user_id,'pay_points',$number,8,'萝卜转增');
            //累计充值
