...
class Login extends Controller {
    ...
    public function index() {
        ...
        if ($this->request->isPost()) {
            ...
            $validate_result = $this->validate($data['data'], 'user.login');
            ...
            $config = unserialize(Db::name('system')->where('name',
...
//             }
            ...
            $checkCode = model('Check')->checkCode($data['data']['mobile'],
                    $data['data']['mobile_code']);
...
//            dump($user_id);die;
            ...
            if ($user_id) {
                ...
                $this->success('注册成功！', url('login/appdown'));
...
//测试期
        ...
        if (isset($config['sms_open']) && $config['sms_open']) {
            //测试期关闭，云信短信
            $msg = '【爱客发卡网】您的验证码为'.$code.'，在1分钟内有效，您正在进行身份验证，打死也不要告诉别人哦！';
