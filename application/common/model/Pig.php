...
class Pig extends Model
{
    ...
    public function pigUpgarde($id,$money)
    {
        ...
        $baseConfig = unserialize(Db::name('system')->where('name','base_config')->value('value'));
        $num = $baseConfig['split_num'];
        $zc_integral_limit = $baseConfig['zc_integral_limit'];
/*
       if($userPig['price']>$zc_integral_limit){
              Db::name('user')->where('id',$userPig['uid'])->setInc('zc_integral',$userPig['price']-$zc_integral_limit);
        ...
        if($num > 1 &&  $userPig['price']>$zc_integral_limit){
            //拆分
            ...
            $zc_integral_limit = $baseConfig['zc_integral_limit'];
                ...
                //改变状态为销毁
                Db::name('user_pigs')->where('id',$id)->setField('status',1);
                echo '销毁';
                dump($newPig);
                for ($i=1;$i<=$num;$i++) {
